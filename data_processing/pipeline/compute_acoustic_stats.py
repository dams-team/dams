# data_processing/pipeline/compute_acoustic_stats.py
...
from utils.artifacts import V1, SEGMENTS_CSV, QC_AUDIO_STATS_CSV

from utils.logger import logger


def preview_rows(ds: Dataset, n: int = 5) -> list[dict]:
    """Debug utility to preview first n rows of a dataset."""
...
def _load_segment_waveform(segments_dir: Path, segment_name: str) -> torch.Tensor:
    ...
    as produced by segment_audio.py.
...
def _log_acoustic_summary(ds_with_flags: Dataset) -> None:
    ...
    for name, count in [
        ...
        logger.info(f'{name}: {count} ({pct:.3f}%)')
...
def main() -> None:
    ...
    segments_dir: Path = settings.segments_path  # segment audio files.
    logger.info(f"Using v1 manifest directory: {v1_dir}")

    # Load segments inventory from v1.
    segments_inventory = v1_dir / SEGMENTS_CSV
    logger.info(f"Loading segments from {segments_inventory}...")
    ds: Dataset = Dataset.from_csv(str(segments_inventory))

    # Each row matches SegmentBaseRow schema (segment_path, raw_file, start_time, end_time).
    # Use preview_rows(ds) for debugging.
    ...
    # Save QC sidecar (not a manifest—these stats don't propagate downstream).
    qc_path = v1_dir / QC_AUDIO_STATS_CSV
    ds_with_flags.to_csv(str(qc_path), index=False)
    logger.info(f"✓ Saved SMAD dataset CSV to {qc_path}")
